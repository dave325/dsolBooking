	...
	class WPDA_Table_Actions {
		...
		public function show() {
			if ( ! isset( $_REQUEST['table_name'] ) || ! isset( $_REQUEST['schema_name'] ) ) {
				...
				<div id="<?php echo esc_attr( $this->table_name ); ?>-tabs" style="border:1px solid #e5e5e5;">
					<div class="nav-tab-wrapper">
						...
						echo '<a id="' . esc_attr( $this->table_name ) . '-sel-1" class="nav-tab nav-tab-active' .
							...
							style="font-size:inherit;">' .
							 ...
							 '</a>';
						echo '<a id="' . esc_attr( $this->table_name ) . '-sel-6" class="nav-tab' .
							 '" href="javascript:void(0)" onclick="settab(\'' . esc_attr( $this->table_name ) . '\', \'6\');" 
							style="font-size:inherit;">' .
						...
						if ( 'Table' === $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-5" class="nav-tab' .
							     '" href="javascript:void(0)" onclick="settab(\'' . esc_attr( $this->table_name ) . '\', \'5\');" 
								style="font-size:inherit;">' .
							     '<span class="dashicons dashicons-editor-code"></span> ' .
					...
					<div id="<?php echo esc_attr( $this->table_name ); ?>-tab-6" style="padding:3px;display:none;">
					...
					if ( 'Table' === $this->dbo_type ) {
						?>
						<div id="<?php echo esc_attr( $this->table_name ); ?>-tab-5" style="padding:3px;display:none;">
		...
		public function tab_settings() {
			...
			$delete_menu_table_form      =
				...
				"<input type='hidden' name='menu_id' id='" . $this->table_name . "_menu_id' value='' />" .
			...
			<table class="widefat striped rows wpda-structure-table">
				<tr>
					<td>
						...
						<div id="wpda_table_settings_<?php echo esc_attr( $this->table_name ); ?>">
							<div style="font-weight:bold; padding-bottom:10px;"><?php echo __( 'These settings only work within in the plugin', 'wp-data-access' ); ?></div>
							<ul style="margin:0; padding:0;">
								<li>
									<span class="wpda_table_settings_caret"><?php echo __( 'Column Labels', 'wp-data-access' ); ?></span>
									<a href="https://wpdataaccess.com/docs/documentation/data-explorer/table-settings/"
									   target="_blank">
										<span class="dashicons dashicons-editor-help"
											  title="<?php echo __( 'Define column labels for list tables and data entry forms [help opens in a new tab or window]', 'wp-data-access' ); ?>"
											  style="cursor:pointer;vertical-align:text-bottom;"></span>
									</a>
									<ul class="wpda_table_settings_nested"
										id="<?php echo esc_attr( $this->table_name ); ?>_setting_1">
										<table class="wpda_table_settings">
											<tr>
												<th><?php echo __( 'Column', 'wp-data-access' ); ?></th>
												<th><?php echo __( 'Label on List Table', 'wp-data-access' ); ?></th>
												<th><?php echo __( 'Label on Data Entry Form', 'wp-data-access' ); ?></th>
											</tr>
											<?php
											...
											foreach ( $this->table_structure as $column ) {
												...
												if ( isset( $columns[ $column['Field'] ] ) ) {
													$label_data_entry = $columns[ $column['Field'] ];
												}
												?>
												<tr>
													<td>
								...
								<li>
									<span class="wpda_table_settings_caret"><?php echo __( 'Column Types', 'wp-data-access' ); ?></span>
									<a href="https://wpdataaccess.com/docs/documentation/data-explorer/table-settings/"
									   target="_blank">
										<span class="dashicons dashicons-editor-help"
											  title="<?php echo __( 'Featured plugin column types [help opens in a new tab or window]', 'wp-data-access' ); ?>"
											  style="cursor:pointer;vertical-align:text-bottom;"></span>
									</a>
									<ul class="wpda_table_settings_nested"
										id="<?php echo esc_attr( $this->table_name ); ?>_setting_2">
										<table class="wpda_table_settings">
											<tr>
												<th><?php echo __( 'Column Name', 'wp-data-access' ); ?></th>
												<th><?php echo __( 'Column Type', 'wp-data-access' ); ?></th>
											</tr>
											<?php
								...
								<li>
									...
									<a href="https://wpdataaccess.com/docs/documentation/data-explorer/add-table-to-dashboard/" target="_blank">
										<span class="dashicons dashicons-editor-help"
											  title="<?php echo sprintf( __( 'Add table `%s` to a WordPress dashboard menu [help opens in a new tab or window]', 'wp-data-access' ), $this->table_name ); ?>"
											  style="cursor:pointer;vertical-align:text-bottom;"></span>
									</a>
									<ul class="wpda_table_settings_nested"
										id="<?php echo esc_attr( $this->table_name ); ?>_setting_3">
										<table class="wpda_table_settings">
											<tr>
												<th>
													<span style="vertical-align:bottom;"><?php echo __( 'Menu Name', 'wp-data-access' ); ?></span>
													<span class="dashicons dashicons-editor-help"
														  title="<?php echo __( 'Name of your sub menu', 'wp-data-access' ); ?>"
												...
												<th>
													...
													<span class="dashicons dashicons-editor-help"
														  title="<?php echo __( 'User roles authorized to see sub menu', 'wp-data-access' ); ?>"
														  style="cursor:pointer;"></span>
												</th>
											</tr>
											<?php
												...
												<tr>
													...
													<td>
														...
														<a href="javascript:void(0)"
														   ...
														   onclick="if (confirm('<?php echo sprintf( __( 'Delete menu %s?', 'wp-data-access' ), esc_attr( $table_menu['menu_name'] ) ); ?>')) { jQuery('#<?php echo $this->table_name; ?>_menu_id').val('<?php echo esc_attr( $table_menu['menu_id'] ); ?>'); jQuery('#<?php echo $delete_menu_table_form_id; ?>').submit(); }"
														></a>
											...
											?>
											<tr id="wpda_add_menu_link" <?php if ( 0 === $no_menus ) {
												echo 'style="display:none;"';
											} ?>>
												<td colspan="3">
													<a class="wpda_table_settings_caret" href="javascript:void(0);"
													   onclick="jQuery('#wpda_add_menu_link').hide(); jQuery('#wpda_add_menu').show();">Add
														menu</a>
												</td>
											</tr>
											<tr id="wpda_add_menu" <?php if ( 0 !== $no_menus ) {
												echo 'style="display:none;"';
											} ?>>
												<td style="vertical-align:top;">
													<input id="menu_name_<?php echo $no_menus; ?>"
							...
							<div style="padding-top:10px;">
								<input class="wpda_table_setting_item" type="hidden" id="sql_dml"
									   value="<?php echo $sql_dml; ?>"/>
								<input type="button"
									   value="<?php echo __( 'Save Table Settings', 'wp-data-access' ); ?>"
									   class="button button-primary"
									   onclick="return submit_table_settings('<?php echo $this->table_name; ?>');">
								<input type="button"
									   value="<?php echo __( 'Reload Table Settings', 'wp-data-access' ); ?>"
									   class="button button-primary"
									   onclick="wpda_show_table_actions_reload( '<?php echo $this->schema_name; ?>', '<?php echo $this->table_name; ?>', '<?php echo $_REQUEST['rownum']; ?>', '<?php echo wp_create_nonce( "wpda-actions-" . $this->table_name ); ?>', '<?php echo $this->dbo_type; ?>' ); return false;">
							</div>
						</div>
					</td>
				</tr>
			...
			<script type='text/javascript'>
				function submit_table_settings(table_name) {
					list_labels = {};
					form_labels = {};
					column_media = {};
					menu = {};
					...
					jQuery('#wpda_table_settings_' + table_name + ' .wpda_table_setting_item').each(function () {
						...
						if (element_id.startsWith("list_label_")) {
							...
							unused[element_id] = element_value;
						}
					});

					jQuery('#wpda_table_settings_' + table_name + ' .wpda_table_setting_item_menu').each(function () {
						element_id = jQuery(this).attr('id');
						element_value = jQuery(this).val();
						element_type = jQuery(this).attr('type');
					...
					jQuery('#<?php echo $settings_table_form_settings; ?>').val(JSON.stringify(jsonData));
					jQuery('#<?php echo $settings_table_form_id; ?>').submit();
					return false;
				}
		...
		protected function tab_actions() {
			...
			<table class="widefat striped rows wpda-structure-table">
				<?php
				$this->tab_export();
				if ( $this->is_wp_table === false && '' !== $this->dbo_type ) {
					$this->tab_rename();
				}
				if ( 'Table' === $this->dbo_type ) {
					$this->tab_copy();
		...
		protected function tab_export() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (<?php echo esc_attr( $check_export_access ); ?>) jQuery('#stealth_mode').attr('src','<?php echo esc_attr( $src ); ?>' + jQuery('#format_type_<?php echo esc_attr( $this->table_name ); ?>').val() + '&include_column_labels=' + (jQuery('#include_column_labels_<?php echo esc_attr( $this->table_name ); ?>').prop('checked') ? 'on' : 'off') + '&include_media_columns=' + (jQuery('#include_media_columns_<?php echo esc_attr( $this->table_name ); ?>').prop('checked') ? 'on' : 'off') + '&include_table_menus=' + (jQuery('#include_table_menus_<?php echo esc_attr( $this->table_name ); ?>').prop('checked') ? 'on' : 'off'))"
					   style="display:block;"
					>
				...
				<td style="vertical-align:middle;">
					...
					<select id="format_type_<?php echo esc_attr( $this->table_name ); ?>"
							name="format_type"
					...
					<?php if ( 'Table' === $this->dbo_type ) { ?>
						<span style="vertical-align: text-top;"><?php echo __( 'Include (SQL only): ', 'wp-data-access' ); ?></span>
						<label style="vertical-align:text-top;">
							<input id="include_column_labels_<?php echo esc_attr( $this->table_name ); ?>"
								   type="checkbox"
								   checked
								   class="wpda_action_font"
							>
							<?php echo __( 'Column labels', 'wp-data-access' ); ?>&nbsp;
						</label>
						<label style="vertical-align:text-top;">
							<input id="include_media_columns_<?php echo esc_attr( $this->table_name ); ?>"
								   type="checkbox"
								   checked
								   class="wpda_action_font"
							>
							<?php echo __( 'Media columns', 'wp-data-access' ); ?>&nbsp;
						</label>
						<label style="vertical-align:text-top;">
							<input id="include_table_menus_<?php echo esc_attr( $this->table_name ); ?>"
								   type="checkbox"
								   checked
								   class="wpda_action_font"
							>
							<?php echo __( 'Table menus', 'wp-data-access' ); ?>&nbsp;
						</label>
					<?php } ?>
				</td>
			</tr>
			...
			<tr>
				...
				<td style="vertical-align:middle;">
					...
					<label style="vertical-align:baseline">
						<input type="checkbox"
							   checked
							   onclick="jQuery('#copy_table_data_<?php echo esc_attr( $this->table_name ); ?>').prop('checked', jQuery(this).is(':checked'));"
							   class="wpda_action_font"
						>
