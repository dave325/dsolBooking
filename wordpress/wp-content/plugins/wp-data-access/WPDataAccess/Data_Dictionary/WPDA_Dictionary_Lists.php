...
namespace WPDataAccess\Data_Dictionary {
	...
	class WPDA_Dictionary_Lists {
		 ...
		 *
		 * jQuery usage: action=get_tables
		 *
		 * @return array
		 */
		public static function get_tables_ajax() {
			if ( isset( $_REQUEST['schema_name'] ) ) {
				$wpdadb = WPDADB::get_db_connection( sanitize_text_field( wp_unslash( $_REQUEST['schema_name'] ) ) );
		...
		public static function get_columns() {
			$schema_name = isset( $_REQUEST['schema_name'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['schema_name'] ) ) : ''; // input var okay.
			$wpdadb = WPDADB::get_db_connection( $schema_name );
			...
			$columns = $wpdadb->get_results( $query, 'ARRAY_A' ); // WPCS: unprepared SQL OK; db call ok; no-cache ok.
		...
		public static function get_table_columns( $table_name, $schema_name ) {
			$wpdbda = WPDADB::get_db_connection( $schema_name );

			$query = $wpdbda->prepare( '
				  SELECT column_name AS column_name
					FROM information_schema.columns 
				   WHERE table_schema = %s
					 AND table_name   = %s
			...
			return $wpdbda->get_results( $query, 'ARRAY_A' ); // WPCS: unprepared SQL OK; db call ok; no-cache ok.
		}
		...
		public static function get_default_collation( $schema_name = '' ) {
			...
			return $wpdadb->get_results(
				"SELECT default_character_set_name AS default_character_set_name,
				 ...
				 FROM   information_schema.schemata
				 WHERE  schema_name = '$wpdadb->dbname'
