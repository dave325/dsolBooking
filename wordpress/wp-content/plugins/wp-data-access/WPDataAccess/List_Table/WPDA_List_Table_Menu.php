...
namespace WPDataAccess\List_Table {
	...
	class WPDA_List_Table_Menu extends WPDA_List_Table {
		...
		protected function process_bulk_action_create_db() {
			...
			if ( isset( $_REQUEST['database_location'] ) ) {
				if ( 'local' === $_REQUEST['database_location'] ) {
					// Add local database
		...
		protected function add_header_button( $add_param = '' ) {
			...
			<div style="text-align: right;">
				...
				<a href="?page=wpda&page_action=wpda_backup&schema_name=<?php echo esc_attr( $this->schema_name ); ?>"
				   class="page-title-action"><?php echo __( 'Schedule Exports (Data Backup)' ); ?></a>
			</div>
			<?php
		...
		protected function get_schema_name() {
			...
			if ( isset( $_REQUEST['wpda_main_db_schema'] ) && '' !== $_REQUEST['wpda_main_db_schema'] ) {
				...
				if ( $this->schema_exists( $_COOKIE[ $cookie_name ] ) ) {
		...
		private function schema_exists( $schema_name ) {
