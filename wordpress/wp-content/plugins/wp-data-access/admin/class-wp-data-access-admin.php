...
class WP_Data_Access_Admin {
	...
	/**
	 * Add some javascript to make sure the plugin help page is shown in a new tab/window
	 */
	public function add_admin_footer() {
		?>
		<script type='text/javascript'>
			jQuery(document).ready(function () {
				jQuery('#wpda_plugin_help').parent().attr('target', '_blank');
			});
		</script>
		<?php
	}
		...
		foreach ( WPDA_User_Menus_Model::list_external_menus() as $menu ) {
			...
			if ( $user_has_role ) {
				if ( ! isset( $menus_shown_to_current_user[ $menu->menu_slug . '/' . $menu->menu_name . '/' . $menu->menu_table_name . '/' . $menu->menu_schema_name ] ) ) {
					...
					$menus_shown_to_current_user[ $menu->menu_slug . '/' . $menu->menu_name . '/' . $menu->menu_table_name . '/' . $menu->menu_schema_name ] = true;
				}
			}
		}
	}
