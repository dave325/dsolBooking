...
namespace WPDataProjects\List_Table {
	...
	class WPDP_List_Table_Lookup extends WPDA_List_Table {
										...
										for ( $i=1; $i<count( $column_options_relationship->source_column_name); $i++ ) {
											if ( isset( $item[ $column_options_relationship->source_column_name[ $i ] ] ) ) {
												...
												if ( 'number' == WPDA::get_type( $data_type_lookup ) ) {
													$where .= $wpdb->prepare( " and {$column_options_relationship->target_column_name[ $i ]} = %d", [ $value ] );
												} else {
													$where .= $wpdb->prepare( " and {$column_options_relationship->target_column_name[ $i ]} = %s", [ $value ] );
												}
											}
										}
		 ...
		 */
		protected function column_lookup( $target_column_name, $target_table_name, $source_column_name, $value, $data_type, $and ) {
			global $wpdb;
			...
			if ( 'number' === WPDA::get_type( $data_type ) ) {
				$sql = $wpdb->prepare( $sql . "%d $and", [ $value ] );
			} else {
				$sql = $wpdb->prepare( $sql . "%s $and", [ $value ] );
			}
